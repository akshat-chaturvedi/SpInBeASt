...
def bess_rv(star_name, file_name):
    ...
    if hdr["BSS_ITRP"] >= 10000:
        ...
        v_bis, v_grid, ccf, gaussian_width = shafter_bisector_velocity(wavs, fluxes)
        ...
        ax[0].vlines(v_bis, 0.15 * max(fluxes - 1), 0.35 * max(fluxes - 1), color="r", zorder=1, lw=3)
        ...
        ax[0].text(0.78, 0.8, fr"{clean_star_name3(hdr['OBJNAME'].strip())} H$\alpha$"
...
list_of_files = list_fits_files("BeSS_HD051354_He")

for file in list_of_files:
    # print(file)
    try:
        bess_rv_he('HD051354', file)
    except:
